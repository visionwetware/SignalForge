...
export default function TopicCard({ topic }) {
  ...
  return (
    <Link
      ...
      className="group block border border-[#E6F7FF]/10 rounded-lg p-5 bg-white/40 hover:bg-white hover:border-[#E6F7FF]/30 transition-all"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="text-[10px] uppercase tracking-[0.18em] text-[#E6F7FF]/50">{topic.category}</div>
          <div className="font-serif text-xl mt-1.5 truncate">{topic.name}</div>
        </div>
        {topic.anomaly_flag && (
          <div className="flex items-center gap-1 text-[10px] uppercase tracking-[0.15em] text-[#FF2BD6] bg-[#FF2BD6]/10 px-2 py-1 rounded">
      ...
      <div className="mt-4 flex items-end justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-[#E6F7FF]/50">Signal</div>
