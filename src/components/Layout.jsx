...
import { Scan, LineChart, BookOpen, FileText, Settings, Shield, LayoutDashboard, Lock, LogOut, Mic, Images } from "lucide-react";
...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] text-[#1A1814] flex">
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-[#F4EFE7]/95 backdrop-blur border-t border-[#1A1814]/8 flex justify-start overflow-x-auto px-2 py-2">
        {NAV.map((item) => {
          const active = location.pathname === item.to || (item.to !== "/" && location.pathname.startsWith(item.to));
          const Icon = item.icon;
          return (
