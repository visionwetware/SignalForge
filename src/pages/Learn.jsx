  ...
  return (
    <div className="max-w-5xl mx-auto px-6 md:px-12 py-12 pb-24">
      <PageHeader
        ...
        description="Sharpen pattern recognition with curated questions on curios, sources, and reasoning."
        action={
          <Link to="/learn/practice" className="inline-flex items-center gap-2 px-5 py-3 bg-[#E6F7FF] text-[#080A18] rounded-md hover:bg-[#E6F7FF]/85 transition">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="border border-[#E6F7FF]/10 rounded-lg p-6 bg-[#0D1026]/70">
          <div className="text-[10px] uppercase tracking-[0.18em] text-[#E6F7FF]/50">Recent sessions</div>
          <div className="font-serif text-4xl mt-2">{sessions.length}</div>
          {sessions.length > 0 ? (
            <ul className="mt-5 space-y-2 text-sm">
              {sessions.map((s) => (
                <li key={s.id} className="flex justify-between text-[#E6F7FF]/70 border-b border-[#E6F7FF]/8 pb-2 last:border-0">
            ...
            <div className="text-sm text-[#E6F7FF]/50 mt-4">No sessions yet — start one above.</div>
          )}
        </div>

        <div className="border border-[#E6F7FF]/10 rounded-lg p-6 bg-[#0D1026]/70">
          <div className="text-[10px] uppercase tracking-[0.18em] text-[#E6F7FF]/50">Accuracy</div>
          <div className="font-serif text-4xl mt-2">{accuracy}%</div>
          {hasFeature(user, "learn_analytics") ? (
            <div className="mt-4 flex items-center gap-2 text-sm text-[#E6F7FF]/70">
              <Trophy className="w-4 h-4 text-[#00F5FF]" />
              Advanced learning analytics enabled.
            </div>
          ) : (
            <div className="mt-4 text-sm text-[#E6F7FF]/55">
