    ...
    <div className="max-w-6xl mx-auto px-6 md:px-12 py-12 pb-24">
      ...
      {reports.length === 0 ? (
        ...
        <div className="grid md:grid-cols-[260px_1fr] gap-6">
          <div className="space-y-1.5">
            {reports.map((r) => (
              <button
                ...
                className={`w-full text-left p-3 rounded-md border transition ${selected?.id === r.id ? "border-[#1A1814] bg-white" : "border-[#1A1814]/10 bg-white/40 hover:bg-white"}`}
              >
                <div className="text-[10px] uppercase tracking-[0.18em] text-[#1A1814]/50">{format(new Date(r.created_date), "MMM d")}</div>
          ...
          {selected && (
            <div className="border border-[#1A1814]/10 rounded-lg bg-white p-8">
              <div className="flex items-start justify-between gap-4 pb-5 border-b border-[#1A1814]/10">
                <div>
                  <div className="text-[10px] uppercase tracking-[0.22em] text-[#1A1814]/50">Report</div>
                  <h2 className="font-serif text-3xl mt-1">{selected.title}</h2>
                </div>
                <button onClick={() => download(selected)} className="inline-flex items-center gap-2 px-4 py-2 text-sm border border-[#1A1814]/15 rounded-md hover:bg-[#1A1814] hover:text-[#FAF7F2] transition">
                  <Download className="w-3.5 h-3.5" /> Markdown
                </button>
              </div>

              <pre className="whitespace-pre-wrap font-mono text-[13px] leading-relaxed mt-6 text-[#1A1814]/85">
{selected.markdown}
              </pre>
            </div>
          )}
