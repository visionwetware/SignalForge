  ...
  return (
    <div className="max-w-6xl mx-auto px-6 md:px-12 py-12 pb-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-14">
        <StatCard to="/scans" label="Scans this month" value={`${used}/${limit}`} hint={`${Math.max(0, limit - used)} remaining`} />
        <StatCard to="/topics" label="Tracked topics" value={topics.length} hint="Active signals" />
        <StatCard to="/topics?filter=anomaly" label="Anomalies" value={topics.filter(t => t.anomaly_flag).length} accent="#FF2BD6" hint="Worth a closer look" />
        <StatCard to="/reports" label="Reports" value={reports.length} hint="Recent intelligence" />
      </div>
      ...
      <section className="mb-14">
        <div className="flex items-baseline justify-between mb-5">
          ...
          <Link to="/scan" className="text-xs uppercase tracking-[0.18em] text-[#E6F7FF]/60 hover:text-[#E6F7FF]">View all →</Link>
        </div>
        {results.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {results.map((r) => (
              <Link key={r.id} to={`/scan/${r.id}`} className="border border-[#E6F7FF]/10 rounded-lg p-5 bg-[#0D1026]/70 hover:bg-white hover:border-[#E6F7FF]/30 transition">
      ...
      <section>
        <div className="flex items-baseline justify-between mb-5">
          <h2 className="font-serif text-2xl">Curio movement</h2>
          <Link to="/topics" className="text-xs uppercase tracking-[0.18em] text-[#E6F7FF]/60 hover:text-[#E6F7FF]">All topics →</Link>
        </div>
        {topics.length === 0 ? (
...
function EmptyState({ text, cta }) {
  return (
    <div className="border border-dashed border-[#E6F7FF]/15 rounded-lg p-10 text-center text-[#E6F7FF]/50 bg-[#0D1026]/50">
