  ...
  return (
    <div className="max-w-6xl mx-auto px-6 md:px-12 py-12 pb-24">
      <PageHeader
        ...
        title={filter === "anomaly" ? "Flagged anomalies" : "Tracked curios"}
      ...
      {topics.length === 0 ? (
        <div className="border border-dashed border-[#E6F7FF]/15 rounded-lg p-10 text-center text-[#E6F7FF]/50">
          No topics yet — run a scan to start tracking curios.
        </div>
      ) : (
        <>
          {focus && (
            <div className="border border-[#E6F7FF]/10 rounded-lg p-6 md:p-8 bg-[#0D1026]/70 mb-8">
              <div className="flex items-baseline justify-between mb-6">
                <div>
                  <div className="text-[10px] uppercase tracking-[0.18em] text-[#E6F7FF]/50">{focus.category}</div>
                  <h2 className="font-serif text-3xl mt-1">{focus.name}</h2>
                </div>
                <div className="text-right">
                  <div className="text-[10px] uppercase tracking-[0.18em] text-[#E6F7FF]/50">Current</div>
                ...
                <div className="h-48">
                  <ResponsiveContainer>
                    <LineChart data={chartData}>
                      <XAxis dataKey="time" stroke="#E6F7FF" strokeOpacity={0.3} fontSize={10} />
                      <YAxis stroke="#E6F7FF" strokeOpacity={0.3} fontSize={10} />
                      <Tooltip contentStyle={{ background: "#E6F7FF", border: "none", borderRadius: 6, color: "#080A18", fontSize: 12 }} />
                      <Line type="monotone" dataKey="value" stroke="#00F5FF" strokeWidth={2} dot={{ r: 3, fill: "#00F5FF" }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              ) : (
                <div className="text-sm text-[#E6F7FF]/60 bg-[#E6F7FF]/5 px-4 py-3 rounded-md">
