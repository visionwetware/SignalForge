...
Deno.serve(async (req) => {
  try {
    ...
    for (const imageUrl of imageUrls) {
      ...
      try {
        await base44.entities.ScanResult.update(scan.id, { status: 'scanning' });
        const analysis = await base44.integrations.Core.InvokeLLM({
          prompt: `Analyze this webpage image for SignalForge. Page title: ${title}. Source URL: ${canonicalUrl}. Return labels, topics, score and concise notes.`,
          file_urls: [imageUrl],
        ...
        await base44.entities.ScanResult.update(scan.id, { status: 'failed', error: error.message });
      }
    }
